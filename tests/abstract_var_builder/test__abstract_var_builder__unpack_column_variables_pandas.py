...
@pytest.mark.parametrize(
    ...
    [
        ...
        pytest.param(
            ...
            TestExpected(expected_data=pd.DataFrame({"result": [1.8, 20.1, 4.0]})),
            id="Case 2: Continuous unpack all",
        ),
        pytest.param(
            TestInputs(
                data=pd.DataFrame(
            ...
            TestExpected(expected_data=pd.DataFrame({"result": [1.8, 0.0, 4.0]})),
            id="Case 3: Partial unpack with default value",
        ),
        pytest.param(
            TestInputs(
                data=pd.DataFrame(
            ...
            TestExpected(expected_data=pd.DataFrame({"result": [False, True, True]})),
            id="Case 4: Binary unpack",
        ),
        pytest.param(
            TestInputs(
                data=pd.DataFrame(
            ...
            TestExpected(expected_data=pd.DataFrame({"result": [3, 0, 4]})),
            id="Case 5: Integer unpack (rounding and casting)",
        ),
    ],
)
def test__unpack_column_variables_pandas__general(inputs, expected):
    """
    Tests whether the _unpack_column_variables_pandas method works as expected.
    """
    # Arrange
    ...
    # Act
    result_df = var_builder._unpack_column_variables_pandas(
        data=inputs.data,
    ...
    # Assert
    pd.testing.assert_frame_equal(result_df, expected.expected_data)


def test__unpack_column_variables_pandas__has_invalid_columns__raises_value_error():
