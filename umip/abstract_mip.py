...
from umip.exception import OptimizationException, AbnormalException, InfeasibleException, UnboundedException
from umip.abstract_dataclasses import AbstractInputData, AbstractOutputData, AbstractInternalData
...
class AbstractMipModel(ABC):
    ...
    def build(self, input_data: AbstractInputData, redirect_solver_log: bool = True, **_) -> None:
        ...
        :param _: Additional arguments that may be used by the builders.
        """
    ...
    def solve(
        self, time_limit: float | None = None, redirect_solver_log: bool = True, keep_variables_data: bool = False
    ) -> None:
        """
        ...
        :param redirect_solver_log: Whether to redirect stdout to a temporary file
        :param keep_variables_data: Whether to keep the variable data in self._internal_data after solving and unpacking.
        """
        if not self._built:
        ...
        if self._solver.is_optimal() or self._solver.is_feasible():
            if keep_variables_data:
                self._internal_unpacked_data = deepcopy(self._internal_data)
            else:
                self._internal_unpacked_data = self._internal_data
    ...
    def _unpack_variables(self) -> None:
        """
        Unpacks the variables after solving the model.
        """

        for variable_builder in self._variable_builders:
            self._internal_unpacked_data = variable_builder.unpack(
                solver=self._solver, data=self._internal_unpacked_data
            )
    ...
    def is_optimal(self) -> float:
        """
        Status of the optimization of the model.
        """
        if not self._solved:
            raise ValueError("Model must be solved before calling .is_optimal()")
    ...
    def get_internal_unpacked_data(self) -> AbstractInternalData:
        """
    ...
    def get_output_data(self, **kwargs) -> AbstractOutputData:
        ...
        if not self._output_data:
            self._output_data = self._convert_internal_to_output_data(
                internal_unpacked_data=self._internal_unpacked_data, **kwargs
            )
    ...
    def _convert_internal_to_output_data(
        self, internal_unpacked_data: AbstractInternalData, **kwargs
    ) -> AbstractOutputData:
        """
